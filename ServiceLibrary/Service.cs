...
namespace ServiceLibrary
{
    public class Service
    {
        ...
        private static readonly string MSMQConfigFile =
            Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), @"MSMQConfig.ini");
        ...
        private static readonly string _labClientService = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), @"LabClient\bin\Debug",
                "LabClient.exe");
        ...
        private Service()
        {
            ...
            MSMQConfig = new MSMQConfig(MSMQConfigFile);
        }
        ...
        // Pings every computer every 30sec, a computer turns Red if ping reply = false
        public void StartPingSvc(List<LabClient> clients)
        {
            new Thread(delegate ()
            {
                while (AppActive)
                {
                    ...
                    //#TODO Should try to find a better way to do this
                    lock (Key)
                    {
        ...
        public void RunRemoteProgram(List<LabClient> compList, string path, string param = "", bool cmdWithQuotes = true)
        {
            foreach (var client in compList)
            {
                ...
                using (var file = new StreamWriter(copyPathRemote))
                {
                    ...
                    runCmd = runCmd + " " + param;
                    line = Config.PsTools + @"\PsExec.exe -s -d -i 1 \\" + compName + @" -u " + _service.Credentials.DomainSlashUser + @" -p " + _service.Credentials.Password + " " + runCmd;
                    file.WriteLine(line);
                }
                _service.StartNewCmdThread(copyPathRemote);
            }
        ...
        public void KillRemoteProcess(List<LabClient> computers, string pcID, bool waitForFinish = false)
        {
            foreach (var client in computers)
            {
                _service.KillRemoteProcessMSG(client.ComputerName, pcID);
                KillProcThread(client.ComputerName, pcID);
            }
                ...
                new Thread(delegate ()
                {
                    ...
                    using (var file = new StreamWriter(copyPathRemote))
                    {
                        ...
                        line = Config.PsTools + @"\PSShutdown.exe \\" + client.ComputerName + @" -u " +
                               _service.Credentials.DomainSlashUser + @" -p " + _service.Credentials.Password +
                               @" -t 20 -m "" Your pc is shutting down in 20s""";
                        file.WriteLine(line);
                    }
                    _service.StartNewCmdThread(copyPathRemote);
                }).Start();
        ...
        public void TransferLabClientService(List<LabClient> labClients)
        {
        
            var t = new Thread(delegate ()
            {

                ProjectFilesAndRunIt(labClients, _labClientService, "LabClient");
            });
            t.IsBackground = true;
            t.Start();
        }
        public void TransferLabClientService(List<LabClient> labClients, Boolean wait)
        {
            var t = new Thread(delegate ()
            {
                // Sleeping in 3 min
        ...
        // Installing messageQueue on selected LabClients
        public void InstallMSMQ(List<LabClient> clients)
        {
            ThreadStart threadStart = delegate
            {
                var cmd = @"cmd /c (dism /online /Enable-Feature /FeatureName:MSMQ-Container /FeatureName:MSMQ-Server /FeatureName:MSMQ-Triggers /FeatureName:MSMQ-DCOMProxy)";
                RunRemoteProgram(clients, cmd, "", false);
        ...
        public void DeleteCookies(List<LabClient> clients)
        {
        ...
        public void DeleteNetworkTempFiles()
        {
            ...
            var snDrive = Path.Combine(Config.Sharednetworkdrive, "temp");
            using (var file = new StreamWriter(batFileName))
            {
                file.WriteLine("@echo off");
